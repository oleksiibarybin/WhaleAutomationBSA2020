...
class PageHelper extends Helper {
    ...
    async loginWithGoogle(email, pwd) {
    ...
    async getIncognitoPage() {
        const context = await this.browser.createIncognitoBrowserContext();
        const incognitoPage = await context.newPage();
        await incognitoPage.goto("https://bsa2020-whale.westeurope.cloudapp.azure.com");
        await this.loginWithGoogleInIncognito("dapofish@gmail.com", "zxc@123456789", incognitoPage);

        await new Promise( r => setTimeout(r, 5000));
        await incognitoPage.waitForSelector('i.bell.outline.icon');
        await incognitoPage.click('i.bell.outline.icon');
        await incognitoPage.waitForSelector('div.dropdown-menu.notify');
        await incognitoPage.waitForSelector('div.ui.small.message');
        await incognitoPage.waitForSelector('button.ui.teal.button');
        await incognitoPage.click('button.ui.teal.button');
        await new Promise( r => setTimeout(r, 3000));
        await incognitoPage.waitForSelector('div.description');
        await incognitoPage.waitForSelector('div.contacts-container');
        await incognitoPage.waitForSelector('div.description');
      
    }

    async loginWithGoogleInIncognito(email, pwd,incognitoPage) {
        //const email = process.env.GOOGLE_EMAIL;
        //const pwd = process.env.GOOGLE_PWD;

        await incognitoPage.waitForSelector('#landing-login');
        await incognitoPage.click('#landing-login');

        await incognitoPage.waitForSelector('#login-google');
        await incognitoPage.click('#login-google');

        // wait needed to wait for new target open, can be refactor to use browser.on('targetcreated')
        await incognitoPage.waitFor(5000);
